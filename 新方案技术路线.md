...
**透传和拦截 需要ai审阅一下项目源代码,看看有没有遗漏的,分辨一下哪些是需要透传的,哪些是需要拦截的**
...
*   **心跳放行**：本地启动后监听 `127.0.0.1:5173`(需要验证是否原后端默认是5173,如果是,这里需要修改为可配置变量,以方便在云端部署)。任何发往 `/api/health` 的探针请求，代理端瞬间返回 200。这立刻满足了 Electron 中对后端的重构存活检测，让前台客户端的 Loading 画面秒速放行启动。
